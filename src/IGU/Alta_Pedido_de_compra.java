...
public class Alta_Pedido_de_compra extends javax.swing.JFrame {
    ...
    public Alta_Pedido_de_compra(JComboBox<String> combooProducto) {
  initComponents();
    ...
    private void btnAltaFacturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAltaFacturaActionPerformed
       ...
       for (Producto_factura p : listaProducto_factura) {
        ...
        for(Producto pro: listaProducto_actualizacionPrecio){
            ...
            cant_de_facturas = servProd.verCantidadDeFacturas(pro.getId_producto())-1;
            System.out.println("C. facturas = "+cant_de_facturas);
            if(cant_de_facturas > 0){
                ...
                prin.llenarComboProducto();
            }else{
                System.out.println("Hola");
                float v1 = (float)(pro.getTotalXFactura() / pro.getCant()) ;
                float v2 = (float)((pro.getTotalXFactura() / pro.getCant()) *1.21);
                float v3 = (float)(((pro.getTotalXFactura() / pro.getCant()) *1.21) * pro.getMark_up());
            servProd.actualizarPrecioProducto(pro.getId_producto(),v1 , v2,v3);
            servProd.actualizarStock(pro.getId_producto(),"alta");
                            prin.llenarComboProducto();
