...
public class ServicioProducto {
    ...
    public boolean altaProducto(String bc,String nombre, int tipo, int marca,String url) {
        try {
            con = cone.conect();
            ...
            if (!rs.next()) {
                pst = con.prepareStatement("INSERT INTO productos (codigo_barra,nombre_producto,precio,mark_up,precio_con_iva,url_imagen,precio_al_publico,id_tipo,id_marca) VALUES ("+bc+",'" + nombre + "', 0 , 1.41 ,0,'"+url+"',0,'" + tipo + "' , '" + marca + "')");
                pst.executeUpdate();
            } else {
                return false;
            }
    ...
    public ArrayList<Producto> listarProductos() {
        ...
        try {
            con = cone.conect();
            pst = con.prepareStatement("SELECT id_producto,codigo_barra,nombre_producto, stock,url_imagen,precio,mark_up,precio_con_iva,precio_al_publico, marca, tipo FROM productos inner join marcas on marcas.id_marca = productos.id_marca inner join tipo_productos on tipo_productos.id_tipo_producto = productos.id_tipo");
            rs = pst.executeQuery();
            while (rs.next()) {
    ...
    public void actualizarStock(int id,String metodo){
        try{
            ...
            if(metodo.equals("alta")){
                System.out.println("Hola desde serrvP");
    ...
    public void actualizacionDePrecioXFactura (int id){
        try {
            con = cone.conect();
            pst = con.prepareStatement("update productos set precio = IF((SELECT pxf.precio_total/pxf.cantidad from productos_x_factura as pxf inner join facturas as f on f.id_factura = pxf.id_factura Where pxf.id_producto = "+id+" order by fecha_factura desc limit 0,1) > (SELECT pxf.precio_total/pxf.cantidad from productos_x_factura as pxf inner join facturas as f on f.id_factura = pxf.id_factura Where pxf.id_producto = "+id+" order by fecha_factura desc limit 1,1),(SELECT pxf.precio_total/pxf.cantidad from productos_x_factura as pxf inner join facturas as f on f.id_factura = pxf.id_factura Where pxf.id_producto = "+id+" order by fecha_factura desc limit 0,1),precio),precio_con_iva = precio * 1.21, precio_al_publico = precio_con_iva * mark_up where productos.id_producto = "+id);
            pst.executeUpdate();
